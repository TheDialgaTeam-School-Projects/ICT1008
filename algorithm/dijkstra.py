...
class Dijkstra:
    ...
    def __init__(self, environment, start, end):
        ...
        self.possibleNodes = PQ()
        self.possibleNodes.insert(self.startNode)
    ...
    def findNewPath(self, currentNode):
        for row in range(-1, 2):
            for column in range(-1, 2):
                ...
                self.possibleNodes.insert(newNode)

                self.visitedNodes.append(newNode)

    def getEndNodeIsInPossibleNodes(self):
        for i in range(1, self.possibleNodes.size + 1):
            if self.endNode == self.possibleNodes.HeapQueue[i]:
                return True
        return False
    ...
    def getEndNodeFromPossibleNodes(self):
        for i in range(1, self.possibleNodes.size + 1):
            if self.endNode == self.possibleNodes.HeapQueue[i]:
                return self.possibleNodes.HeapQueue[i]
        return None

    def resetPath(self, startNode, endNode):
        self.visitedNodes = []

        self.possibleNodes.clear()
        self.possibleNodes.insert(startNode)
    ...
    def update(self):
        if self.possibleNodes.isEmpty():
            ...
            if not self.getEndNodeIsInPossibleNodes():
                ...
                self.findNewPath(self.possibleNodes.deleteMin())
            else:
